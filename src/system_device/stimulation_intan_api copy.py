...
from src.enum.constants import (
 ...
 │        # 断开 TCP 连接
 │
 ├─── 【状态查询与校验层】确保 RHX 处于可接收参数的状态 ─────
 │    ├── verify_controller_type()
 │    │   # 校验控制器为 StimRecord 类型，否则抛异常
 │    ├── ensure_controller_stopped()
 │    │   # 查询 RunMode，若运行中则发送 stop 指令
 │    └── get_SampleRateHertz(scommand, ...)
 │        # 查询当前采样率（Hz）
 │
 ├─── 【参数构建层】拼装 TCP 命令字符串，不直接发送 ────────
 │    ├── _configureStimulation(...)
 │    │   # 完整参数构建：双相电流幅值、脉宽、放大器稳定时间、
 │    │   # 数字输出 TTL → 拼接命令串 + uploadstimparameters
 │    ├── configureTrainStimulation(...)
 │    │   # 脉冲串专用，固定单相幅值 + 脉冲数
 │    ├── configureSingleStimulation(...)
 │    │   # 单脉冲专用，固定单相幅值
 │    └── _get_duration_ttl_by_spec(stim_spec)
 │        # 根据刺激类型×位置(左/右/全局) 查表返回 TTL 脉宽
 │
 ├─── 【参数下发层】调用构建层 + 通过 TCP 发送至 RHX ────────
 │    ├── configure_stimulation(channels, ...)
 │    │   # 多通道参数下发入口：遍历通道列表，
 │    │   # 调用 _configureStimulation → sendall 发送命令串
 │    └── cancel_config(channels)
 │        # 批量禁用通道刺激，重置稳定时间与不应期后上传
 │
 ├─── 【执行控制层】运行模式切换与触发 ────────
 │    ├── startRun() / stopRun()
 │    │   # 切换 RHX 运行模式 run ↔ stop
 │    ├── startRecord() / stopRecord()
 │    │   # 切换 RHX 运行模式 record ↔ stop
 │    └── trigger_stimulation(key)
 │        # 发送 manualstimtriggerpulse 触发已配置的刺激
 │
 └─── 【辅助】
      └── enableDigitalInChanel(digital_in)
          # 启用数字输入通道（用于外部触发信号读取）


  ┌─────────────────────────────────────┐
  │  Phase 1     Phase 2       ISI      │
  │  200 μs      200 μs      9600 μs    │
  │  -75 mV      +75 mV        0 mV     │
  └─────────────────────────────────────┘
   amplitude = [ -75000,  +75000,    0    ]  (μV)
   duration  = [   200,     200,   9600   ]  (μs)
...
class StimulationIntan(object):
    ...
    def configureSingleStimulation(self, scommand, channel, source, amplitude, duration):
        ...
        com_firstphasedurationmicroseconds = f'set {channel}.firstphasedurationmicroseconds {duration};'
        com_uploadstimparameters = f'execute uploadstimparameters {channel};'
        com_config = com_stimenabled + com_source + com_firstphaseamplitudemicroamps
        + com_firstphasedurationmicroseconds + com_uploadstimparameters;
    ...
    def _get_duration_ttl_by_spec(self, stim_spec: StimulationSpec) -> int:
        ...
        return duration_mapping.get(combination_key, 100)  # 默认1000μs
    ...
    def trigger_stimulation(self, key):
        ...
        time.sleep(0.1)  # 给刺激执行一段时间 给了100ms来施加刺激，确保100ms内的刺激都施加完毕
    ...
    def startRun(self):
        self.connect_socket.sendall(b'set runmode run;')
        time.sleep(0.05)  # Give some time for the command to be processed
