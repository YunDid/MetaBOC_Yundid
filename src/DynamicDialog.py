...
from src.Ui_DynamicDialog import Ui_Dynamic
...
class DynamicDialog(QDialog, Ui_Dynamic):
    ...
    def __init__(self, parent=None, para=None, spike_para=None, recording=None, stimulating=None):
        ...
        self.stimulating.stimulate_finshed.connect(self.stimulate_once_finished)    # 一次刺激完成
        ...
        self.start_end_change(0)    # 开始有刺激信号传入，就需要自动设置时间限制
        ...
        self.dynamic_model_run_time = self.spb_model_run_time.value()    # 用于设定模型运行的时间，即数据采集的时间
        self.last_frame_num = 0      # 记录上一次的数据位置
        self.is_record_data = False  # 是否在记录数据
        ...
        self.pb_load_model.clicked.connect(self.load_dynamic_model)
        self.spb_model_run_time.valueChanged.connect(self.model_runtime_changed)
        # self.pb_stop.clicked.connect(self.stop_dynamic_process)

    def initial_dynamic_model(self):
        ...
        ckpt_json = 'src/dynamic_model/checkpoints/test-0123.json'
        ckpt_path = 'src\dynamic_model\checkpoints\0123-data.json'
        
        recording_list = self.para.recording_list
        sti_list = self.para.stimulating_list
        ...
        # 以左侧电极测试
        self.dynamic_model_run = NeuroDynamicModel(ckpt_json, ckpt_path,recording=self.recording)
    ...
    # 更新模型运行的剩余时间
    def update_dynamic_model_run_time(self):
        re_time = self.dynamic_model_run_time - (time.time() - self.start_mark)    # 单位s
        self.spb_model_run_time.setValue(int(re_time))

    def reset_dynamic_model_run_time(self):
        self.spb_model_run_time.setValue(self.dynamic_model_run_time)

    def model_runtime_changed(self, value):
        if self.spb_model_run_time.isEnabled():
            self.dynamic_model_run_time = self.spb_model_run_time.value()
            print("...........")

    def stimulate(self):
        # self.check_sti_or_dynamic()
        self.stimulating.set_sti_signal(self.para)   # 每次刺激开始，需重置初始刺激信号
        ...
        print("Stimulate new times...............................")

        self.start_mark = time.time()
        self.spinBox_countdown.setValue(self.last_time_once_sti)
        self.pb_stimulate.setEnabled(False)
        self.pb_close.setEnabled(False)
        self.spb_model_run_time.setEnabled(False)
        ...
        self.timer.start(1000)    # 每秒定时更新剩余刺激时间

    # 一次刺激完成，开始进行
    def stimulate_once_finished(self, state):
        if state:
            self.timer.stop()
            self.timer.start(250)                          # 每250ms记录一次数据

            print("once stimulate finished...................................................")
            self.recording.clear_recording_buffer(True)    # 刺激完成时，开始记录数据
            self.is_record_data = True
    ...
    def get_recording_data_after_once_sti(self):  
        ...
        #     print("Sti not finished!...")
        #     return
        if not self.is_record_data:
            self.update_remain_time()
            self.update_dynamic_model_run_time()
            self.timer.start(1000)
            print("Not recording data!...")
            return
        else:
            print("Recordinig data!...")

        # 以下为记录信号数据，以及进行动力学模型推理过程
        electrode_key = self.recording.channel_map

        # t1 = time.time()  获取特定时间内的所有通道数据,channel_data在本次刺激中一直累加
        resolution, channel_data  = copy.deepcopy(self.recording.get_target_time_signal())

        # 获得刺激后特定时间内的多段数据，每次获取新的spike数据，已经获取过的数据会自动跳过
        # out_raw_data = self.get_spike_data_from_channel_data(resolution, channel_data)

        out_raw_data = channel_data[:, self.last_frame_num:]
        self.last_frame_num = len(channel_data[0, :]) - 1
        print("frame mark", self.last_frame_num)
        
        # 需要保存一段时间内的，每个电极的spike个数
        for i in electrode_key:    # 遍历所有通道，计算每个通道的spike
            # 获取
            spikes = []
            if i == "15":
                self.one_mean_map[i].append(0.0)
                continue
            index = electrode_key[i]   # i 表示key,index是实际的

            # 分段处理方式，（刺激之后的固定时刻）
            # for j in range(len(out_raw_data)):
            #     y = out_raw_data[j]
            #     data = y[index - 1]  # channel 从0开始，需减1
            #     spike_num = self.spike_detection_thread.run(data)
            #     spikes.append(spike_num)

            # 不区分去刺激时刻
            data = out_raw_data[index - 1]  # channel 从0开始，需减1
        ...
        print("spike detection finished!...")

        # 单次刺激信号刺激完成，最后显示各通道的总和spike个数；同时启动Dynamic生成刺激信号
        if time.time() - self.start_mark > self.last_time_once_sti and time.time() - self.start_mark < self.dynamic_model_run_time:   
            self.spinBox_countdown.setValue(0)    # 初始人工设计的刺激已经结束
            self.update_dynamic_model_run_time()

            # 判断采集的数据点个数，从而启动model dynamic
            if len(self.one_mean_map["47"]) < 25:
                print("Num of spike time:", len(self.one_mean_map["47"]))
                self.timer.start(250)
            else:               
                self.dynamic_model_sti(self.one_mean_map)
        ...
        elif time.time() - self.start_mark > self.dynamic_model_run_time:    # 模型预测刺激，返回数据
            self.recording.clear_recording_buffer(False)   # 不再记录数据
            self.is_record_data = False
            ...
            self.spb_model_run_time.setEnabled(True)
            self.timer.stop()
    ...
    def load_dynamic_model(self):
        checkpoint, ok_ = QFileDialog.getOpenFileName(self, "Open Model", "C:/", filter="*.ckpt", options=QFileDialog.DontUseNativeDialog)

        if checkpoint:
            file_name = os.path.basename(checkpoint)
            self.lineEdit_model.setText(file_name)
            
            self.dynamic_model.load_model(checkpoint_path=checkpoint)
            print("Load dynamic model successful!...")

    # 检查相应的模型或刺激参数是否设置，自动加载模型时，不需要判断
    def check_sti_or_dynamic(self):
        model_text = self.lineEdit_model.text()
        sti_signal = self.lineEdit_stimulating_mode.text()

        if model_text == "" or sti_signal == "":
            return False
        else:
            return True
    ...
    def dynamic_model_sti(self, spikes):
        ...
        self.dynamic_model_run.set_input_spike_data(cur_spikes)
        amplitude, duration = self.dynamic_model_run.run()    # 单位： μV   μs
        ...
        self.stimulating.update_amplitude_duration(amplitude, duration)
    ...
    #     electrode_key = self.recording.channel_map
    #     channel_key_spike = copy.copy(self.recording.channel_map)
    #     for i in channel_key_spike:
    #         channel_key_spike = []
    ...
    #     for i in electrode_key:    # 遍历所有通道，计算每个通道的spike； i表示key
    #         if i == "15":
    #             continue
    ...
    #             data = y[index - 1]  # channel 从0开始，需减1
    #             spike_num = self.spike_detection_thread.run(data)
    #             channel_key_spike.append(spike_num)
        
    #     return channel_key_spike
        
    # # def stop_dynamic_process(self):
    # #     self.recording.clear_recording_buffer(False)   # 不再记录数据
