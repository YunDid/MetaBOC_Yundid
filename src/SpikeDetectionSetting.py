...
from src.Ui_SpikeDetectionSetWidget import Ui_SpikeDetectionSetting

from src.infor_com_mea.spike_detection import SpikeDetection 


class SpikeDetectionSetting(QWidget, Ui_SpikeDetectionSetting):
    close_widget = pyqtSignal(bool)

    def __init__(self, parent=None, para=None, recording=None):
        super(SpikeDetectionSetting, self).__init__(parent)
        self.setupUi(self)
        ...
        if para is not None:
            self.spb_filter_order.setValue(para["Filter_Order"])
            self.spb_sample_rate.setValue(para["Sample_Rate"])
            self.spb_max_fre.setValue(para["Max_Frequency"])
            self.spb_min_fre.setValue(para["Min_Frequency"])
            self.spb_refrac_time.setValue(para["Refractor_Time"])
            self.spb_multiplier.setValue(para["Multiplier"])

        self.spike_detection_thread = SpikeDetection(
            sample_rate=self.spb_sample_rate.value(), order=self.spb_filter_order.value(),
            min_fre=self.spb_min_fre.value(), max_fre=self.spb_max_fre.value(), 
            multiplier=self.spb_multiplier.value(), period=self.spb_refrac_time.value())
        ...
        self.timer.timeout.connect(self.show_spike_result)
        self.timer.start(1000)

        self.spb_sample_rate.valueChanged.connect(self.update_spike_para)
        self.spb_filter_order.valueChanged.connect(self.update_spike_para)
        self.spb_max_fre.valueChanged.connect(self.update_spike_para)
        self.spb_min_fre.valueChanged.connect(self.update_spike_para)
        self.spb_multiplier.valueChanged.connect(self.update_spike_para)
        self.spb_refrac_time.valueChanged.connect(self.update_spike_para)
    ...
    def update_spike_para(self, value):
        self.spike_detection_thread.update_para(sample_rate=self.spb_sample_rate.value(), order=self.spb_filter_order.value(),
            min_fre=self.spb_min_fre.value(), max_fre=self.spb_max_fre.value(), 
            multiplier=self.spb_multiplier.value(), period=self.spb_refrac_time.value())

    def ok(self):
        self.para = {"Sample_Rate":self.spb_sample_rate.value(),
            "Filter_Order":self.spb_filter_order.value(),
            "Min_Frequency":self.spb_min_fre.value(),
            "Max_Frequency":self.spb_max_fre.value(),
            "Multiplier":self.spb_multiplier.value(),
            "Refractor_Time":self.spb_refrac_time.value()}
        self.close()
    ...
    def get_spike_det_para(self):
        return self.para

    def show_spike_result(self):
        electrode = self.recording.channel_map

        for i in electrode:
            if i == "15":
                continue
            index = electrode[i]
            x, y = self.recording.get_one_second_signal(index)
            self.spike_detection_thread.set_data(y)
            self.spike_detection_thread.start()
